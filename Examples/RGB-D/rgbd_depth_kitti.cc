                ...
                vector<string> &vstrImageFilenamesD, vector<double> &vTimestamps)
{
    ...
    ifstream fTimes;
    string strPathTimeFile = strPathToSequence + "image_02/timestamps.txt";
    ...
    string strPrefixRGB = strPathToSequence + "image_02/data/";
    string strPrefixDepth = strPathToSequence + "image_02_Depth_blacktop/";

    const int nTimes = vTimestamps.size();
    vstrImageFilenamesRGB.resize(nTimes);
    vstrImageFilenamesD.resize(nTimes);

    for (int i = 0; i < nTimes-10; i++){
        stringstream ss;
        ss << setfill('0') << setw(10) << i+5;
